...
@Component({
  selector: 'app-parkhand-member',
  templateUrl: './parkhand-member.component.html',
  styleUrls: ['./parkhand-member.component.css']
})
export class ParkhandMemberComponent {
  ...
  get_data(){
    this._crud.get_user().subscribe(
      (res: any) => {
        console.log(res);
        const data = res.data.filter((item: any) => item.member.includes('प्रखण्ड'));
